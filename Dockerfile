...
COPY "ghost/core/ghost-$GHOST_VERSION.tgz" .
...
RUN set -eux; \
	...
	aptPurge=; \
	\
	installCmd='gosu node ghost install --archive "/ghost-$GHOST_VERSION.tgz" --db mysql --dbhost mysql --no-prompt --no-stack --no-setup --dir "$GHOST_INSTALL"'; \
